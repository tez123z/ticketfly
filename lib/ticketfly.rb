module Ticketfly
  ...
  class Headliner
    attr_accessor :id, :name, :json, :twitterScreenName
    def self.build(json)
      headliner = Headliner.new
      headliner.id = json['id']
      headliner.name = json['name']
      headliner.twitterScreenName 
      headliner.json = json
      headliner
    end
  end
  ...
  class Event
    attr_accessor :id, :name, :venue, :org, :date, :json, :ticketPurchaseUrl, :urlEventDetailsUrl, :headlinersName, :supportsName, :image, :startDate, :endDate, :doorsDate, :onSaleDate, :offSaleDate, :ticketPrice, :urlEventDetailsUrl
