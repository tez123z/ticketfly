module Ticketfly
  ...
  class Event
    attr_accessor :id, :name, :venue, :org, :date, :json, :ticketPurchaseUrl, :urlEventDetailsUrl, :headlinersName, :supportsName, :image, :startDate, :endDate, :doorsDate, :onSaleDate, :offSaleDate, :ticketPrice, :urlEventDetailsUrl, :showType, :showTypeCode
    ...
    def self.build(json)
      ...
      event.image = json['image']['square']['path']
  ...
  class Events
    ...
    def self.search(query)
      ...
      page = 1
      begin
        result = JSON.parse(open(base_uri + "?orgId=1&q=" + query.to_s + "&maxResults=" + max_results).read)
        total_pages = result["totalPages"]
        result['events'].each do |e|
          event = Event.build(e)
          events << event
